        ...
        <section class="page-section">
          ...
          >
            ...
            <template v-slot="{ list }">
              <ul class="article-list w-full max-w-screen-xl grid grid-cols-3 gap-4 align-center mx-auto my-2">
                <li v-for="article in list" :key="article._path" class="article">
                  <NuxtLink :to="article._path">
                    <div class="wrapper ">
                      <div class="img-cont w-32 ">
                        <img :src="`/${article.img}`" :alt="article.title" class="rounded-lg max-h-[8rem] h-full object-cover" />
                      </div>
                      <header class="pl-2">
                        <h1 class="text-2xl font-semibold">{{ article.title }}</h1>
                        <p>{{ article.description }}</p>
                        <ul class="article-tags">
                          <li class="bg-gray-400  text-gray-200 px-1 py-0.5" v-for="(tag, n) in article.tags" :key="n">{{ tag }}</li>
                        </ul>
                      </header>
                    </div>
                  </NuxtLink>
